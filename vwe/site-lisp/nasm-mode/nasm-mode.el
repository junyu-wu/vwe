...
(eval-and-compile
  (defconst nasm-pp-directives
    '("%elif" "%elifn" "%elifctx" "%elifnctx" "%elifdef" "%elifndef"
      ...
      "%warning" "%xdefine" "istruc" "at" "iend" "align" "alignb" "vstart"
...
(defconst nasm-number-regexp
  ;; (rx symbol-start (1+ num) symbol-end)
  "\\_<[[:digit:]]+\\_>"
  "Regexp for `nasm-mode' for matching numeric constants.")

(defconst nasm-binary-regexp
  "\\<[01_]+[01_B]\\>"
  "Regexp for `nasm-mode' for matching numeric constants.")

(defconst nasm-number-special-format-regexp
  "\\<?[\-\\|\+\\|\*\\|\/\\|=]+[0x[:xdigit:]\\|[:digit:]]+\\>"
  "Regexp.")
...
(defmacro nasm--opt-words (keywords)
  "Prepare KEYWORDS for `looking-at'."
  `(eval-when-compile
...
(defconst nasm-font-lock-keywords
  `((,nasm-section-name-regexp (1 'nasm-section-name))
    ...
    (,(nasm--opt-words nasm-instructions) . 'nasm-instructions)
    (,(nasm--opt-words nasm-pp-directives) . 'nasm-preprocessor)
    (,(concat "^\\s-*" nasm-nonlocal-label-rexexp) (1 'nasm-labels))
    (,(concat "^\\s-*" nasm-local-label-regexp) (1 'nasm-local-labels))
    (,nasm-number-regexp . 'nasm-constant)
    (,nasm-binary-regexp . 'nasm-constant)
    (,nasm-number-special-format-regexp . 'nasm-constant)
    (,nasm-constant-regexp . 'nasm-constant)
    (,(nasm--opt nasm-directives-list) . 'nasm-directives))
