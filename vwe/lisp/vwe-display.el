...
;; `whitespace'
;;
(vwe@lib--pkg whitespace
  :init ((add-hook 'after-init-hook #'global-whitespace-mode))
  :variable ((setq whitespace-line-column nil
				   whitespace-style '(lines lines-tail))))

;;
;; `highlight-indent-guides' 显示缩进样式
;;
(vwe@lib--pkg highlight-indent-guides
  :init ((add-hook 'prog-mode-hook #'highlight-indent-guides-mode))
  :config ((defun vwe@pkg--highlighter-func (level responsive display)
			 "Highlighter function, whit LEVEL RESPONSIVE and DISPLAY."
			 (if (> 1 level)
				 nil
			   (highlight-indent-guides--highlighter-default level responsive display))))
  :variable ((setq highlight-indent-guides-method 'character
...
;; `all-the-icons'
;;
(vwe@lib--pkg all-the-icons)

;;
;; `desktop'
;;
(vwe@lib--pkg desktop
  :init ((when vwe@custom--frame-save-and-recover-layout?
  ...
  :config ((when vwe@custom--frame-save-and-recover-layout?
			 (add-to-list 'desktop-modes-not-to-save 'dired-mode)
			 (add-to-list 'desktop-modes-not-to-save 'Info-mode)
			 (add-to-list 'desktop-modes-not-to-save 'info-lookup-mode)
			 (add-to-list 'desktop-modes-not-to-save 'fundamental-mode)
			 (advice-add #'desktop-owner :around #'vwe@pkg--desktop-owner-advice)))
  :variable ((setq desktop-path (append (list (vwe@lib--path-cache "desktop/"))
										desktop-path)
...
(vwe@lib--pkg winum
  ...
  :config ((vwe@lib--keymap-global-set '(("M-~" winum-select-window-by-number)
  ...
  :variable ((setq winum-auto-setup-mode-line nil)))
